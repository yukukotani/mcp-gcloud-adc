...
import { McpProxyHandler, createMcpProxy } from './handler.js';
...
describe('McpProxyHandler', () => {
  ...
  describe('handleRequest', () => {
    it('成功したリクエストを処理する', async () => {
      ...
      expect(mockAuthClient.getIdToken).toHaveBeenCalledWith('https://example.com/api');
    ...
    it('予期しないエラーを処理する', async () => {
      ...
      vi.mocked(mockAuthClient.getIdToken).mockRejectedValue(new Error('Unexpected error'));
    ...
    it('verboseモードでログを出力する', async () => {
      ...
      expect(mockStderr).toHaveBeenCalledWith(expect.stringContaining('Proxying request: tools/list'));
      expect(mockStderr).toHaveBeenCalledWith(expect.stringContaining('Response received for: tools/list'));
    });
  });
  ...
  describe('handleMessage', () => {
    ...
    it('通知の認証エラーを処理する', async () => {
      ...
      expect(result).toEqual(notification);
      expect(mockStderr).toHaveBeenCalledWith(expect.stringContaining('Authentication failed for notification'));
    });
  });
});
