...
const parseTimeout = (value?: string): number | null => {
...
const parseLogType = (value?: string): "pretty" | "json" | "file" | null => {
  if (!value) return null;
...
const loadConfig = (
  ...
  return {
    ...
    proxy: {
      url: cliOptions.url,
      timeout: parseTimeout(env.MCP_PROXY_TIMEOUT) || cliOptions.timeout,
    },
    auth: {
      ...(env.GOOGLE_APPLICATION_CREDENTIALS && {
    ...
    logging: {
      level: parseLogLevel(env.MCP_PROXY_LOG_LEVEL) || "info",
      type: parseLogType(env.LOG_TYPE) || "file",
...
const validateConfig = (config: AppConfig): ConfigValidationResult => {
  ...
  if (
    ...
    !isValidPath(config.auth.credentialsPath)
  ) {
...
export function createConfigManager(packageInfo: {
  ...
  return {
    loadConfig: (cliOptions: ProxyOptions) =>
      loadConfig(packageInfo, cliOptions),
    validateConfig,
