...
export class DefaultConfigManager implements ConfigManager {
  ...
  validateConfig(config: AppConfig): ConfigValidationResult {
    ...
    // 認証設定の検証（警告レベル）
    if (config.auth.credentialsPath && !this.isValidPath(config.auth.credentialsPath)) {
  ...
  private parseLogLevel(value?: string): 'debug' | 'info' | 'warn' | 'error' | null {
    if (!value) return null;
...
export function createConfigManager(packageInfo: { name: string; version: string }): ConfigManager {
